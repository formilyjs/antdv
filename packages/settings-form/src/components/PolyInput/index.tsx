...
export function createPolyInput(polyTypes: PolyTypes = []) {
  return defineComponent({
    name: 'DnPolyInput',
    props: ['value', 'exclude', 'include', 'onChange'],
    emits: ['change'],
    setup(props, { emit }) {
      ...
      const getNextType = () => {
        const currentIndex = types?.findIndex(({ type }) => type === unref(current))
            ...
            {typeComponent && (
              <div class={prefixRef.value + '-content'}>
                <typeComponent
                  {...{
                    ...props,
                    value: type?.toInputValue ? type?.toInputValue(props.value) : props.value
                  }}
                  onChange={(event: any) => {
