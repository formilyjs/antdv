...
import { isNum, uid } from '@pind/designable-shared'
import { IconWidget, TextWidget, usePrefix, useTheme } from '@formily/antdv-designable'
...
const MonacoInputInner = defineComponent({
  ...
  setup(props, { attrs, emit }) {
    ...
    const updateExtraLib = () => {
      ...
      extraLibRef.value = monacoRef.value!.languages.typescript.typescriptDefaults.addExtraLib(
        props.extraLib!,
        `${uidRef.value}.d.ts`
      )
    }
    ...
    return () => {
      ...
      realLanguage.value = /(?:javascript|typescript)/gi.test(computedLanguage.value)
        ? 'typescript'
        : computedLanguage.value
      return (
        <div
